...
@router.get("/")
async def get_metrics(company_id: Optional[str] = None, db: AsyncSession = Depends(get_db)):
    # 1. Total & Completed Tasks
    total_tasks_res = await db.execute(select(func.count(Task.id)))
    total_tasks = total_tasks_res.scalar() or 0

    completed_tasks_res = await db.execute(select(func.count(Task.id)).where(Task.status == "COMPLETED"))
    ...
    active_agents_res = await db.execute(select(func.count(Agent.id)).where(Agent.status == "WORKING"))
    ...
    # 4. Approvals Count
    pending_approvals_res = await db.execute(select(func.count(Approval.id)).where(Approval.status == "PENDING"))
    pending_approvals = pending_approvals_res.scalar() or 0

    # 5. Financial Transactions
    revenue_res = await db.execute(
        select(func.sum(FinancialTransaction.amount)).where(FinancialTransaction.transaction_type == "REVENUE")
    )
    ...
    # Recent Transactions
    tx_res = await db.execute(select(FinancialTransaction).order_by(FinancialTransaction.created_at.desc()).limit(10))
...
async def create_transaction(payload: Dict[str, Any] = Body(...), db: AsyncSession = Depends(get_db)):
