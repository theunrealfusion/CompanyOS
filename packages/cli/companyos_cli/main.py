        ...
        async with httpx.AsyncClient() as client:
            try:
                resp = await client.post(
                    "http://localhost:8003/api/v1/companies/",
                    params={"name": name, "mission": mission},
                )
                if resp.status_code == 200:
                    data = resp.json()
                    click.secho(
                        f"✅ Created Company '{data['name']}' with ID {data['id']}", fg="green"
                    )
                else:
        ...
        async with httpx.AsyncClient() as client:
            try:
                resp = await client.get(
                    f"http://localhost:8003/api/v1/companies/{company_id}/settings"
                )
                if resp.status_code == 200:
                    data = resp.json()
